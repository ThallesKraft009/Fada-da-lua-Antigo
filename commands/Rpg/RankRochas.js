...
module.exports = {
  name: "rank-pedra",
  aliases: [''],
    description: "Veja o rank de pedra",
    usage: '',
    category: 'Ranks',
    cooldown: 5000,
  	userPerms: [''],
  ...
  run: async (client, message, args) =>{
    ...
    usersDB.sort((a,b) => b.rpg.blocos.pedra - a.rpg.blocos.pedra)
    ...
    ctx.fillText(`${usersDB[i].rpg.blocos.pedra} pedras`, 300, cordenada + 150)
...
let botao = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
					.setCustomId(`proximaPg_Pedra_2_${message.author.id}`)
  .setEmoji("▶️")
...
message.reply({content: `<a:bop:788210473454796861>  | Carregando..`}).then(msg => {
    ...
    setTimeout(() => {
    const attachment = new Discord.AttachmentBuilder(canvas.toBuffer(), {name: 'rankPedra.png'}) 
